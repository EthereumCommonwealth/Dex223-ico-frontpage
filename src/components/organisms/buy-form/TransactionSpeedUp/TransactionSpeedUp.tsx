...
function useGasEstimation({
  ...
  const estimatedGasFee = useMemo(() => {
    return (+formatEther(
      (maxBaseFee + maxPriorityFee) * BigInt(transactionToSpeedUp?.details.gas),
    )).toFixed(7);
...
function SpeedUpVariant({
  ...
  return (
    <div className={styles.speedUpLabelWrapper}>
      <label
        className={clsx(
          styles.speedUpLabel,
          id === "custom" && styles.noBorder,
...
export default function TransactionSpeedUp({ handleClose }) {
  ...
  const { transactionToSpeedUpId, speedUpType, setType, setTransactionToSpeedUp } =
    useTransactionSpeedUp();
  ...
  const [customBaseFee, setCustomBaseFee] = useState(
    addBigIntPercent(BigInt(transactionToSpeedUp.details.maxFeePerGas), 10),
  );
  const [customPriorityFee, setCustomPriorityFee] = useState(
    addBigIntPercent(BigInt(transactionToSpeedUp.details.maxPriorityFeePerGas), 10),
  );
  ...
  const { write: handleSpeedUp } = useContractWrite({
    ...purchaseConfig,
  ...
  return (
    <>
      ...
      />
      {!isMetamask ? (
        <div className={styles.metamaskMessageContainer}>
          <Image src="/images/wallets/metamask.svg" alt="" width={80} height={75} />
          <h3>Try the solution built into Metamask</h3>
          <p>
            We have noticed that you are using MetaMask wallet. Unfortunately it does not support
            Speed Up through dApps. As an alternative you could use built-in speed up option inside
            Metamask extension. Here is{" "}
            <TextLink
              text="more details"
        ...
        <div className={styles.speedUpContent}>
          <RecentTransaction
            noSpeedUp
            transaction={transactions[address].find((t) => {
            ...
            [
              {
                ...
                helperText:
                  "Sets the transaction fee to 10% higher than the original value. The minimum acceptable option for speeding up a transaction.",
              },
              {
                id: "market",
                value: marketData.estimated,
                helperText:
                  "Configures transaction values according to the current state of the network.",
              },
              {
                id: "aggressive",
                value: aggressiveData.estimated,
                helperText:
                  "Sets transaction fee values based on the values from the last block with an additional increase, which guarantees that the transaction will be included in the next block but may result in higher fee.",
              },
              {
                id: "custom",
                value: customData.estimated,
                helperText:
                  "This option allows you to set the transaction fee values manually. Keep in mind that the fee must exceed the original values by at least 10% in order for a transaction to be accepted on the network.",
              },
            ] as { id: TransactionSpeedUpType; value: string; helperText: string }[]
          ).map((variant) => {
            return (
              <SpeedUpVariant
                id={variant.id}
                handleCheck={() => setType(variant.id)}
                key={variant.id}
                isActive={speedUpType === variant.id}
                calculatedValue={variant.value}
          ...
          <div
            className={clsx(
              styles.customSpeedUpSettingsWrapper,
              speedUpType !== "custom" && styles.disabled,
            )}
          >
            <div className={clsx(styles.customSpeedUpSettings)}>
              <div className={styles.labelInputWrapper}>
                ...
                <div className={styles.helperText}>
                  Current <span className="desktop">transaction</span>{" "}
                  {formatEther(BigInt(+transactionToSpeedUp.details.maxFeePerGas), "gwei")} Gwei
                </div>
              </div>
              <div className={styles.labelInputWrapper}>
                <label>
                ...
                <div className={styles.inputWrapper}>
                  <NumericFormat
                    ...
                    onValueChange={(values) =>
                      setCustomPriorityFee(parseEther(values.value, "gwei"))
                    }
                ...
                <div className={styles.helperText}>
                  Current{" "}
                  {formatEther(BigInt(+transactionToSpeedUp.details.maxPriorityFeePerGas), "gwei")}{" "}
                  Gwei
                </div>
              </div>
            </div>
            {customBaseFeeError && speedUpType === "custom" && (
              <div className={styles.feeError}>
                <Svg iconName="error" />
                {customBaseFeeError}
              </div>
            )}
          ...
          <Button
            disabled={
              (Boolean(customBaseFeeError) && speedUpType === "custom") ||
              transactionToSpeedUp.status !== RecentTransactionStatus.PENDING
            }
