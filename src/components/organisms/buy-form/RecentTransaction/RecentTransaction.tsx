...
function TransactionRow({
  ...
  return (
    <div className={clsx(styles.transactionRow, noSpeedUp && styles.withWrapper)}>
      {transaction.title}
      <div className={styles.transactionIcons}>
        {transaction.status === RecentTransactionStatus.PENDING && !noSpeedUp && (
          <button
            ...
            onClick={() => setTransactionToSpeedUp(transaction.id)}
          >
            Speed up
          </button>
        )}
        ...
        <a target="_blank" href={getExplorerLink(transaction.hash, transaction.chainId)}>
          <Svg iconName="forward" />
        </a>
        <span className={transaction.status === RecentTransactionStatus.ERROR && styles.error}>
          {transactionStatusImage[transaction.status]}
        </span>
      </div>
    </div>
  );
