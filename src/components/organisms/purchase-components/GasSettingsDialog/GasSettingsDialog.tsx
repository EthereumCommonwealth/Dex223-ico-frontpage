...
export default function GasSettingsDialog({ isOpen, onClose }: Props) {
  const { type, setDefaultType, setLegacyType } = useTransactionTypeStore();
  ...
  return (
    <DrawerDialog isOpen={isOpen} onClose={onClose}>
      <div className={styles.dialog}>
        <DialogHeader
          ...
          paragraph={
            <span>
              Network fee is paid when you submit a transaction. We recommend changing Priority Fee
              only in order to make transaction cheaper or speed it up at a cost of paying higher
              fee. There are two types of transactions in Ethereum: EIP-1559 and Legacy. Network Fee
              = gasLimit * (Base Fee + Priority Fee) for EIP-1559 transactions. Network Fee =
              gasLimit * gasPrice for Legacy transactions.{" "}
              <span className="desktop">
                We recommend using EIP-1559 transactions on any chain except BSC. BSC does not
                support EIP-1559 transactions so use Legacy there.
              </span>
            </span>
          }
        />
        <div className={styles.priceSettingContainer}>
          <div className={styles.tabButtonsContainer}>
            <div className={styles.tabButtons}>
              <button
                className={clsx(styles.tabButton, type === "default" && styles.active)}
              ...
              <button
                className={clsx(styles.tabButton, type === "legacy" && styles.active)}
                onClick={setLegacyType}
              >
                Legacy
              </button>
            </div>
          ...
          {type === "default" && (
            <>
                  ...
                  <div className={styles.inputWrapper}>
                    <NumericFormat
                      ...
                      onValueChange={(values) =>
                        setMaxPriorityFeePerGas(parseEther(values.value, "gwei"))
                      }
              ...
              {gasFeeValidation.error && (
                <AlertMessage text={gasFeeValidation.error} severity="error" />
              )}
        ...
        <div className={styles.gasLimitSettings}>
          <div className={styles.labelInputWrapper}>
            ...
            <div className={styles.inputWrapper}>
              <NumericFormat
                ...
                className={clsx(
                  ...
                  gasLimitValidation.warning && styles.warning,
                  gasLimitValidation.error && styles.error,
                )}
                placeholder="Gas limit"
                onValueChange={(values) => setUnsavedGasLimit(BigInt(values.value))}
                type="text"
                value={isEditing ? unsavedGasLimit.toString() : gasLimit.toString()}
            ...
            {gasLimitValidation.warning && (
              <AlertMessage text={gasLimitValidation.warning} severity="warning" />
            )}
            {gasLimitValidation.error && (
              <AlertMessage text={gasLimitValidation.error} severity="error" />
            )}
          </div>
        </div>
      </div>
