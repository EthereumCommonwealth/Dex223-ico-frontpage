...
export default function useTrackFeeData() {
  const {
    setBaseFee,
    setMaxFeePerGas,
    computed: baseFeeComputed,
    focused: gasFeeFocused,
  } = useTransactionGasFee();
  const {
    setBasePriority,
    setMaxPriorityFeePerGas,
    computed: priorityComputed,
  } = useTransactionPriorityFee();
  const {
    setGasPrice,
    setBaseGasPrice,
    computed: gasPriceComputed,
    focused: gasPriceFocused,
  } = useTransactionGasPrice();
  ...
  useEffect(() => {
    ...
    setMaxPriorityFeePerGas,
    gasFeeFocused,
    baseFeeComputed.customized,
    gasPriceFocused,
    gasPriceComputed.customized,
  ]);
}
