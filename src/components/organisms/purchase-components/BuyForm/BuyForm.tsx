...
export default function BuyForm({ presale = false }: { presale?: boolean }) {
  ...
  const { isUnViewed, pending, failed, success, totalUnViewed } = useRecentTransactionTracking();
  ...
  return (
    <div className={styles.formToBuy}>
      ...
      <div className={styles.ratio}>
        <span>
          1 D223 = $
          {(1 / +D223Price).toLocaleString("en-US", {
      ...
      {presale && (
        <div className={styles.min}>Min. purchase amount is $5000 in the private sale</div>
      )}

      <TokenCard
        withPicker
        presale={presale}
        balance={tokenToPayBalance?.formatted}
        type="pay"
        tokenName={pickedToken.symbol}
        tokenLogo={pickedToken.image}
        amount={amountToPay}
        handleChange={(v) => setAmountToPay(v)}
      />
      <Spacer height={12} />
      <TokenCard
        balance={D223Balance?.formatted}
        type="receive"
        tokenName={DEX223.symbol}
        tokenLogo="/images/tokens/D223.svg"
        amount={output}
        handleChange={null}
        readonly
      />
      ...
      <div className={clsx(styles.gasSettings, gasSettingsOpened && styles.gasSettingsOpened)}>
        <div
          className={styles.gasHeader}
          role="button"
          onClick={() => setGasSettingsOpened(!gasSettingsOpened)}
        >
          ...
          */}
          ...
          <div className={styles.gasExpand}>
            <Svg iconName="gas" />~ {networkFee}
            <button className={styles.editGasButton} onClick={() => setGasSettingsOpened(true)}>
              EDIT
            </button>
          </div>
        </div>
        ...
        <>
          {Boolean(failed.length) && (
            <AlertMessage
              text={
                <div>
                  Your recent transaction(s) failed. Click{" "}
                  <button
                    onClick={() => setRecentTransactionsOpened(true)}
          ...
          {Boolean(pending.length) && (
            <AlertMessage
              text={
                <div>
                  We have noticed that you have a pending transaction, you can track or speed it up
                  below
                  <span
                    style={{
                      marginLeft: 6,
      ...
      <div className={styles.recentTransactionsField}>
        <div className="relative">
          ...
          Recent transactions
          {Boolean(isUnViewed) && <span className={styles.unreadCounter}>{totalUnViewed}</span>}
        </div>
      ...
      <RecentTransactionsDialog
        isOpen={isRecentTransactionsOpened}
        handleClose={() => {
