...
export default function PurchaseActionButton({
  ...
  const { isLoading, purchaseWithCoins, purchaseWithTokens } = usePurchaseTokens({ presale });
  ...
  const { isOpened: confirmInWalletDialogOpened, setIsOpened: setConfirmInWalletDialogOpened } =
  ...
  const { validation: feeValidation } = useTransactionGasFee();

  const { validation: gasPriceValidation } = useTransactionGasPrice();

  const { validation: gasLimitValidation } = useTransactionGasLimit();
  ...
  if (!isConnected) {
    return (
      <>
        ...
        <Spacer height={20} />
        <Button onClick={openKeystore} variant="outlined">
  ...
  if (Boolean(chain?.id) && chain.id !== chainToConnect.id) {
    return (
      <Button onClick={() => switchNetwork(chainToConnect.id)}>
  ...
  if (waitingForApprove || isLoading) {
    return (
      <>
        ...
        <Dialog
          isOpen={confirmInWalletDialogOpened}
          onClose={() => setConfirmInWalletDialogOpened(false)}
        >
          <div className="flex flex-col items-center justify-center p-10 relative">
            <button
              onClick={() => setConfirmInWalletDialogOpened(false)}
              className="absolute right-2 top-2 text-secondary-text hover:text-primary-text"
            >
              <Svg iconName="close" />
            </button>
            <Preloader size={48} />
  ...
  if (
    allowanceData < parseUnits(amountToPay, pickedToken.decimals) &&
    !isNativeToken(pickedToken)
  ) {
    return (
      <Button
        onClick={() => {
    ...
    <Button
      onClick={() => {
        if (+output > +contractBalance) {
          showMessage(
            `There are not enough tokens for sale. Just wait for the next round.`,
            "info",
          );
          return;
        }
