...
import { comparativeTableData } from "./parameters";
      ...
      <div className={clsx("container_internal", styles.competitorsTableWrapper)}>
        ...
        <div className={styles.competitorsTable}>
          {imagesRow.map((image, index) => {
            return (
              <div key={index} className={clsx(styles.cell, index !== 0 && styles.centered)}>
                {image}
              </div>
            );
          })}
          ...
          {comparativeTableData.map((row, rowIndex) => {
            return row.map((value, index) => (
              <div key={index} className={clsx(styles.cell, rowIndex % 2 === 0 && styles.light)}>
                {value}
              </div>
            ));
          })}
          ...
          <div
            className={clsx(styles.dexBorder, entry?.isIntersecting && styles.animate)}
            ref={ref}
          />
        </div>
      </div>
