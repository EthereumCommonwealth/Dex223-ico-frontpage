...
import { useAccount, useBalance, useBlockNumber, useSwitchChain, useWalletClient } from "wagmi";
...
export default function UpgradePage() {
  ...
  const handleUpgrade = useCallback(async () => {
    if (!walletClient) {
      addToast(t("toasts.walletClientNotFound"), "error");
      return;
    }

    if (!D223Balance?.value) {
      addToast(t("toasts.balanceEmpty"), "error");
      return;
    }
  ...
  }, [D223Balance?.value, walletClient, t]);
  ...
  return (
    <Container>
      <Pattern
        ...
        className="w-150 h-150 -left-[290px] -top-[120px]"
      />
      <Pattern
        patternColor={PatternColor.GREEN}
        className="w-150 h-150 -right-[390px] -bottom-[290px]"
      />
        ...
        <div className="w-full px-4">
              ...
              {isConnected ? (
                ...
                <div className="mt-6 flex items-center flex-col sm:flex-row justify-center gap-3">
                  <div className="flex items-center gap-3">
                    <Button onClick={() => setOpenedWallet(true)}>{t("connectWallet")}</Button>
                  </div>
                </div>
              )}
            </>
          ...
          {status === "error" && (
            <div className="flex flex-col items-center">
              <Image width={160} height={160} src="/status/error.svg" alt={t("statusImageAlt")} />
              <h2 className="text-primary-text text-24 ">{t("error.title")}</h2>
          ...
          {status === "success" && (
            <div className="flex flex-col items-center">
              <Image width={160} height={160} src="/status/success.svg" alt={t("statusImageAlt")} />
              <h2 className="text-primary-text text-24 mb-2 font-medium">{t("success.title")}</h2>
              <p className="text-center text-secondary-text mb-6 max-w-[420px] mx-auto">
